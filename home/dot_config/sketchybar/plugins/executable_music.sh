...
# Credentials are only needed to report the song upstream; the bar itself works
# without them, so a fresh machine still shows what is playing.
DAIKIE_SONGPLAYED_URL=""
DAIKIE_TOKEN=""
if [[ -f "$DAIKIE_FILE" ]]; then
    DAIKIE_SONGPLAYED_URL=$(grep "DAIKIE_SONGPLAYED_URL" "$DAIKIE_FILE" | cut -d'=' -f2- | tr -d '[:space:]')
...
if [[ -f "$INFO_FILE" ]] && [[ -s "$INFO_FILE" ]]; then
    ...
    if [[ -n "$TITLE" ]]; then
        ...
        if [[ "$CURRENT_HASH" != "$LAST_HASH" ]]; then
            ...
            # Send to server (skipped if not configured)
            if [[ -n "$DAIKIE_SONGPLAYED_URL" ]] && [[ -n "$DAIKIE_TOKEN" ]]; then
